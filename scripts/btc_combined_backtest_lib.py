"""
btc_combined_backtest.py  â  v5.0
==================================
Nine strategies combining:
  A) signals_backtesting_ml.py  â 31 on-chain/macro signals, ternary ML walk-forward
  B) btc_regime_backtest.py     â technical signals + walk-forward combinatorics

Strategies:
  S1  Signals Alone           â exact signals_backtesting_ml.py methodology
      31 CryptoQuant on-chain/macro signals â ternary scoring (730-day rolling
      percentile) â monthly Spearman top-10 selection â equal-weight composite
      â sigmoid cash mapping (20â50%) â BTC exposure (â50â80%)

  S2  Signals + Combinatorics â S1 signal universe + walk-forward pair ranking
      Same 31 signals, ternary scored â 6-month walk-forward combinatorics â
      pair-power composite â BTC exposure

  S3  Full Regime + All Data  â all signals (A+B) + standard combinatorics
      S1 (31) + technical (~30) signals â combined 6-month walk-forward
      combinatorics â BTC exposure

  S4  MVRV-Sensitive Regime   â S3 + MVRV-regime-filtered training window
      Training window restricted to dates in same MVRV Z-score regime

  S9  S6 + Confidence Gate    â S6 signal universe + S8 execution policy
      99-signal state+delta universe â MVRV-weighted pairs â gated hold-previous sizer

  BH  BTC Buy & Hold          â benchmark

Signal convention (consistent across both source files):
  Positive composite â bearish (increase cash / reduce BTC exposure)
  Negative composite â bullish (reduce cash / increase BTC exposure)

Requirements:
  pip install yfinance pandas numpy matplotlib scipy requests
  CRYPTOQUANT_KEY env var  â optional (enables on-chain signals; proxies used without it)
  FRED_API_KEY env var     â optional (enables real HY Spread)
"""
...
# --- Combinatorics engine (from btc_regime_backtest.py â unchanged) ---
...
S1_PERCENTILE_HI     = 80    # Upper threshold â +1 (bearish)
S1_PERCENTILE_LO     = 20    # Lower threshold â -1 (bullish)
...
# Wide variant: 0â50% cash â â52â98% BTC exposure
...
PRESCREEN_N     = 20              # top-N signals to keep before pair-scoring (reduces C(114,2)=6441 â C(20,2)=190)
GATE_SWEEP_LEVELS = [0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.40, 0.50]

# --- v5.0 Module experiment constants ---
# Module C: Î³ for conviction weighting within the dead-zone-preserving mapping.
# gamma=2.0: hr=0.60â0.04, hr=0.75â0.25, hr=0.90â0.64, hr=1.0â1.0
# gamma=1.0: hr=0.60â0.20, hr=0.75â0.50, hr=0.90â0.80, hr=1.0â1.0 (linear)
CONVICTION_GAMMA = 2.0
V5_VARIANTS = {           # label â (primary_horizon, use_magnitude, conviction_gamma)
...
RETRAIN_MONTHS = 3   # quarterly rebalancing (6M â 13 rebalances; 3M â 25 rebalances)

# --- Naive benchmarks for each strategy group ---
# S1/S2 have mid_exposure=0.65 â benchmark: fixed 65% BTC
# S3-S8 have mid_exposure=0.75 â benchmark: fixed 75% BTC
STRATEGY_BENCHMARK = {
    "Base":                   "BTC 75% Fixed",
...
# Set to "1" via env var to run the retrain frequency study (slow: 4Ã S4 runs)
...
# ============================================================
# 3. PROXY SIGNALS  (yfinance / computed â no API key needed)
# ============================================================
...
def compute_proxy_signals(close: pd.Series,
    ...
    # MVRV proxy: Mayer Multiple (price / 200-day MA) â high â overvalued â normal
    ...
    # S&P 500 trend: % above/below 200-day MA â inverse (bullish when above)
...
# ============================================================
# 'normal':  high raw value â +1 (bearish)
# 'inverse': high raw value â -1 (bullish)

SIGNAL_DIRECTION = {
    # CryptoQuant â valuation (high = overbought = bearish)
    "MVRV":          "normal",
    "SOPR":          "normal",
    "SOPR_Adj":      "normal",
    "STH_SOPR":      "normal",
    ...
    "NVM":           "normal",
    "S2F_Dev":       "normal",
    # CryptoQuant â on-chain activity (high = distribution = bearish)
    "Exch_Reserve":  "normal",
    "Exch_Netflow":  "normal",
    "Whale_Ratio":   "normal",
    "MPI":           "normal",
...
def compute_ternary_matrix(raw_df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert raw signal values â ternary (+1, 0, -1) using rolling percentiles.
    ...
    If direction='normal':  raw > p_hi â +1 (bearish), raw < p_lo â -1 (bullish)
...
def run_s1_walkforward(raw_df: pd.DataFrame,
    ...
    Returns: (composite: pd.Series, selection_history: dict[date â list[str]])
    """
    ...
    selection_history = {}  # rd.date() â list of selected signal names
    ...
    for i, rd in enumerate(rebalance_dates):
        ...
        for col in ternary_df.columns:
            # Validity filter: raw signal must have â¥70% non-NaN data in window
...
def s1_composite_to_exposure(composite: pd.Series,
    ...
    """
    S1 cash â exposure mapping:

      cash% = cash_min + (cash_max - cash_min) * sigmoid(k * composite)

    Base (50â80%):  cash_min=0.20, cash_max=0.50, k=3.0
    Wide (â52â98%): cash_min=0.00, cash_max=0.50, k=3.0
    """
    def _sigmoid(x):
...
# ============================================================
# 9. TECHNICAL SIGNAL CLASSES  (from btc_regime_backtest.py â unchanged)
# ============================================================
...
class PercentileGapSignal(BaseSignal):
    ...
    More regime-adaptive than fixed Â±0.3 threshold: normalizes by the historical
    distribution of gaps, so a 20% gap scores differently in bull vs bear markets.
    High gap (above 80th pctile of gap history) â +1 (bearish).
    """
    ...
    def compute(self, close: pd.Series, extra: dict = None) -> pd.Series:
        ...
        # rolling percentile rank â [0, 1] â center to [-1, +1]
...
def build_technical_signal_registry():
    """
    Principled signal universe â 18 configs (32 â 18):

    MA Breakout (both SMA + EMA):
      Daily crypto-native : [30, 90, 180, 365]  â 1m / 1q / 6m / 1y (crypto calendar)
      Daily traditional   : [50, 100, 200, 365]  â 50/200 industry standard + midpoint + crypto annual
      Weekly              : [4, 13, 26, 52]       â same horizons in weekly bars

    Z-Score (both SMA + EMA):
      Daily               : 50, 90, 200, 365      â traditional + crypto quarter/year
      Weekly              : 26                    â semi-annual

    SP500 macro regime (both SMA + EMA):
      [50, 200] â golden cross; traditional equity calendar retained for SP500
    """
    sigs = []
    ...
    # Percentile Gap â magnitude-aware, regime-adaptive (6 variants)
    for mt in ["SMA", "EMA"]:
...
# ============================================================
# 11. COMBINATORICS ENGINE  (from btc_regime_backtest.py â unchanged)
# ============================================================
...
_REGIME_WEIGHT = {0: 1.0, 1: 0.5, 2: 0.1}   # distance 0/1/2+ â weight
...
def _regime_weights(mvrv_tr: pd.Series, current_regime: str) -> pd.Series:
    ...
    Same regime â 1.0, adjacent â 0.5, two steps away â 0.1.
    """
    try:
...
def pair_power(disc_window, fwd_window, s1, s2, cutoff, weights=None,
    ...
    use_magnitude:   if True, multiply score by median(|ret|)/realized_vol
                     (Module M â rewards pairs that predict large moves).
    ...
    if primary_horizon not in fwd_window.columns:
        return best
    # min_ev scaled to horizon: 90d requires 3Ã more observations
...
def score_at_date(disc, fwd, top_pairs, t, conviction_gamma=None):
    ...
    weight: sv = (2*|hr-0.5|)^gamma, scaled so hr=0.60âsmall, hrâ1.0âlarge.
...
def build_combo_score_wf(score_df: pd.DataFrame,
    ...
    Returns: (combo: pd.Series, pair_history: dict[date â list[tuple]])
    """
    disc  = discretize_signals(score_df)
    fwd   = compute_forward_returns(close)
    sigs  = list(score_df.columns)
    # FIX 2: removed full-sample activity filter (act/asigs/pairs) â moved inside loop
    ...
    if conviction_gamma: mods.append(f"C(Î³={conviction_gamma})")
    ...
    pair_history = {}   # rd.date() â list of (s1, s2) top pairs
    ...
    for i, rd in enumerate(rebalance_dates):
        ...
        # FIX 2: per-window activity filter â prevents full-sample lookahead bias
        ...
        top_lbl   = f"{top_pairs[0][0][:15]}â¦" if top_pairs else "none"
        print(f"    [{rd.date()} â {next_rd.date()}] pairs={len(loop_pairs)}, top1: {top_lbl}", flush=True)
...
# ============================================================
# 12. S4 â MVRV REGIME-SENSITIVE WALK-FORWARD COMBINATORICS
# ============================================================
...
def build_combo_score_wf_mvrv(score_df: pd.DataFrame,
    ...
    At each rebalance date rd:
      ...
      4. [Optional] Pre-screen to top-prescreen_n signals by solo Spearman with 30d return
         (reduces search space from C(n,2) to C(prescreen_n,2) â critical for large universes)
    ...
    fwd   = compute_forward_returns(close)
    sigs  = list(score_df.columns)
    # FIX 3: removed full-sample activity filter (act/asigs/pairs) â moved inside loop
    ...
    for i, rd in enumerate(rebalance_dates):
        ...
        # FIX 3: per-window activity filter â prevents full-sample lookahead bias
        act_tr   = {s: (disc_tr[s] != 0).mean() for s in sigs}
        asigs_tr = [s for s, a in act_tr.items() if a > 0.05]
        pairs_tr = list(combinations(asigs_tr, 2))
...
def analyze_signal_stability(s1_hist: dict,
    ...
    if s1_hist:
        ...
        print(f"\nS1 â signal turnover: {to:.1%} per period  "
    ...
    if s2_hist:
        ...
        print(f"\nS2 â pair turnover: {to:.1%} per period  "
    ...
    if s3_hist:
        ...
        print(f"\nS3 â pair turnover: {to:.1%} per period  "
    ...
    if s4_hist:
        ...
        print(f"\nS4 â pair turnover: {to:.1%} per period  "
    ...
    if s5_hist:
        ...
        print(f"\nS5 â pair turnover: {to:.1%} per period  "
    ...
    if s6_hist:
        ...
        print(f"\nS6 â pair turnover: {to:.1%} per period  "
...
# S2: S1 signals + combinatorics â use S1-aligned range
S2_CFG = StrategyConfig(
    "S2 (Signals+Combos)",
    min_exposure=0.50, mid_exposure=0.65, max_exposure=0.80,
    confidence_threshold=0.20)

# S3: All signals + combinatorics â wider regime range
S3_CFG = StrategyConfig(
    "S3 (All+Combos)",
    min_exposure=0.50, mid_exposure=0.75, max_exposure=1.00,
    confidence_threshold=0.20)

# S4: MVRV-sensitive â same range as S3
S4_CFG = StrategyConfig(
    "S4 (MVRV-Regime)",
    min_exposure=0.50, mid_exposure=0.75, max_exposure=1.00,
    confidence_threshold=0.20)

# S5: Delta-only signals â same bounds as S4 (MVRV framework)
S5_CFG = StrategyConfig(
    "S5 (Delta-only)",
    min_exposure=0.50, mid_exposure=0.75, max_exposure=1.00,
    confidence_threshold=0.20)

# S6: State + Delta signals â same bounds as S4 (MVRV framework)
...
# ============================================================
# 14. EXPOSURE MAPPING  (for S2/S3/S4 â tanh sigmoid with dead-zone)
# ============================================================

def signal_to_exposure_scalar(s: float, cfg: StrategyConfig) -> float:
    """
    Scalar version: maps a single combo value â [-1, +1] â BTC exposure â [min, max].
...
def signal_to_exposure(signal: pd.Series, cfg: StrategyConfig) -> pd.Series:
    """
    Maps combo signal â [-1, +1] â BTC exposure â [min, max].
    Dead zone: |signal| < confidence_threshold â mid_exposure (no trade).
...
def tiered_exposure(combo: pd.Series, cfg: StrategyConfig,
    ...
    """
    S7: Maps combo â discrete 3-tier exposure (step function).
    ...
    |combo| < thresholds[0]                 â neutral (mid_exposure)
    thresholds[0] â¤ |combo| < thresholds[1] â moderate (midÂ±half-range)
    |combo| â¥ thresholds[1]                 â strong (max or min)
    """
    ...
    def _tier(c):
        if np.isnan(c) or abs(c) < thresholds[0]:
            ...
            if c < 0:   # moderate bull â halfway between mid and max
                return (cfg.mid_exposure + cfg.max_exposure) / 2
            else:       # moderate bear â halfway between min and mid
...
def gate_level_sweep(combo: pd.Series, cfg: StrategyConfig, close: pd.Series,
    ...
    For each gate level g â gate_levels:
    ...
    The implied win-rate proxy maps |combo| gate to historical hit-rate:
      implied_wr% â 50 + gate Ã 50  (e.g., gate=0.20 â ~60% win rate)

    Reuses the existing backtest engine â no re-scoring of signal pairs needed.
...
def bootstrap_sharpe_diff(ret_a: pd.Series, ret_b: pd.Series,
    ...
    Returns observed diff, p-value (fraction of resamples where B â¤ A), 90% CI.
    """
...
def print_layer_conclusion(layer: str, base_label: str, new_label: str,
                           ...
                           note: str = "") -> None:
    sig = "â significant (p<0.10)" if boot["significant"] else "â not significant"
    print(f"\n{'='*70}")
    print(f"  LAYER CONCLUSION â {layer}")
    print(f"  {base_label}  â  {new_label}")
...
def full_metrics(cum: pd.Series, daily: pd.Series,
    ...
    if naive_ar is not None:
        diff = ar - naive_ar
        sign = "+" if diff >= 0 else ""
        flag = "â" if diff > 0 else ("=" if abs(diff) < 0.001 else "â")
...
COLORS = {
    ...
    # Layer 2 â delta
    "Base+Delta":                "#FF5722",
    # Layer 3 â MVRV regime
    "Base+MVRV":                 "#F44336",
    "Base+Delta+MVRV":           "#FF9800",
    # Layer 4 â execution policies
...
def plot_strategy_comparison(backtest_results: dict, title: str, out_path: str):
    ...
    if non_bh:
        ...
        if "exposure" in bt0.columns:
            ...
            ax.set_ylabel(f"Exposure signal â {name0.strip()}")
...
def plot_signal_heatmap(ternary_df: pd.DataFrame,
    ...
    axes[0].set_title(f"Top {n_top} On-Chain/Macro Signal Heatmap  (ternary Â±1/0)")
...
def run_retrain_frequency_study(combined_df: pd.DataFrame,
    ...
    Varies: retrain_months â how often pair rankings are refreshed.

    Key trade-offs:
      1M  â most responsive but highest pair churn; overfits recent noise
      3M  â quarterly; adapts within a market phase; recommended middle ground
      6M  â current default; 13 OOS periods over 6.2 years; can lag regime shifts
      12M â very stable pair selection; may miss entire bear/bull phase

    Prints: Ann.Ret, Sharpe, Calmar, Max DD, Fees, Pair Turnover, N rebalances.
    Conclusion printed at the end.
    """
    print("\n" + "=" * 90)
    print("RETRAIN FREQUENCY STUDY  â S4 (MVRV-Regime)")
    print(f"Signals: {combined_df.shape[1]}  |  Train window: 3Y  |  Top-N pairs: 15")
    print(f"Tested intervals: 1M / 3M / 6M / 12M  (eval period: {EVAL_START} â)")
    print("=" * 90)
    ...
    print("\n" + "=" * 90)
    print("RETRAIN FREQUENCY STUDY â RESULTS")
    print("=" * 90)
    ...
    print(f"\nConclusion:")
    print(f"  Best Sharpe  â {best_sh['Interval']} ({best_sh['Sharpe']})")
    print(f"  Best Ann.Ret â {best_ar['Interval']} ({best_ar['Ann.Ret']})")
...
def main():
    ...
    print("  L2: +Delta  â do N-day difference signals add net value?")
    print("  L3: +MVRV   â does regime conditioning add net value?")
    print("  L3b-PS: +Delta+MVRV w/ pre-screening (top-20 Spearman â 190 pairs)")
    print("  L4: +Exec   â Tiered / Gate / Tiered+Gate on final base")
    print("  Sweep: Gate sensitivity [0.05 â 0.50] on Base+MVRV combo")
    print("=" * 80)
    ...
    print(f"BTC range: {close.index[0].date()} â {close.index[-1].date()}")

    # ------------------------------------------------------------------ #
    # B. CryptoQuant signals â REQUIRED
    # ------------------------------------------------------------------ #
    print("\n[2/8] On-chain signals (CryptoQuant)...")
    cquant_token = os.environ.get("CRYPTOQUANT_KEY")
    if cquant_token:
        print("  API key found â fetching live on-chain signals")
    else:
        print("  No CRYPTOQUANT_KEY set â attempting to load from cache...")
    ...
    if len(cq_signals) < 5:
        ...
        print("  â¢ Only 10 proxy/macro signals are available (intended: 34+).")
        print("  â¢ S4 MVRV regime weighting uses Mayer Multiple proxy,")
        print("    which has the wrong scale for the Z-score regime bounds.")
        print("  â¢ All strategy results are misleading and not comparable.")
        print()
    ...
    print(f"  Delta signals: {len(delta_raw_df.columns)} "
          f"({len(delta_cols)} on-chain/proxy Ã {DELTA_LOOKBACKS})")
    ...
    # ------------------------------------------------------------------ #
    # E. Technical signals â cached by last BTC date
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # F. Walk-forward combinatorics  (all 4 universe Ã regime combos)
    # ------------------------------------------------------------------ #
    ...
    if mvrv_raw is None:
        print("  WARNING: MVRV Z-score not available â regime weighting disabled")
    ...
    print(f"    Reduces C({s6_df.shape[1]},2)={s6_df.shape[1]*(s6_df.shape[1]-1)//2} pairs "
          f"â C({PRESCREEN_N},2)={PRESCREEN_N*(PRESCREEN_N-1)//2} pairs per rebalance")
    ...
    # ââ v5.0: Module Experiment (V0âV5) âââââââââââââââââââââââââââââââââ #
    print("\n\n" + "â"*70)
    ...
    # Execution layer â applied to Base+MVRV combo (L3 winner from prior runs)
    ...
    print("\n" + "=" * 130)
    print(f"OVERALL PERFORMANCE  [{EVAL_START} â {close.index[-1].date()}]"
          f"  (retraining: {RETRAIN_MONTHS}M  |  naive: BTC 75% Fixed)")
    print("=" * 130)
    print(mdf.to_string(index=False))
    print("=" * 130)
    print("vs Naive: â = beats fixed-allocation baseline  â = fails to beat it")
    ...
    print("\n[L3b] Base+MVRV  â  Base+Delta+MVRV")
    ...
    print_layer_conclusion("L3b: + Delta on top of MVRV base", "Base+MVRV", "Base+Delta+MVRV",
                           ...
                           note=f"Base+Delta+MVRV has {s6_df.shape[1]} signals / {s6_df.shape[1]*(s6_df.shape[1]-1)//2} pairs â high search space risk")
    ...
    for label, oos in [("Base+MVRV+Tiered",     tiered_oos),
        ...
        fees = oos["fee_drag"].sum()
        print(f"\n  Base+MVRV  â  {label}  (fees: {fees:.2%}/yr)")
    ...
    print("\n[L4b] Base+Delta+MVRV+Gate (PS20)  â prescreened large universe")
    ...
    print_layer_conclusion("L4b: Pre-screened Delta+MVRV+Gate",
                           ...
                           note=f"PS20 reduces {s6_df.shape[1]*(s6_df.shape[1]-1)//2} â {PRESCREEN_N*(PRESCREEN_N-1)//2} pairs; "
                                f"gate fees: {fees_ps:.2%}/yr")

    # --- Gate level sensitivity sweep ---
    print("\n--- Gate Level Sensitivity (on Base+MVRV combo) ---")
    print(f"    Mapping: |combo| gate â implied win-rate proxy (50 + gateÃ50)")
    gate_sweep_df = gate_level_sweep(bmvrv_combo, S4_CFG, close)
    print(gate_sweep_df.to_string(index=False))

    # ââ v5.0 Module Experiment Results ââââââââââââââââââââââââââââââââââ #
    print("\n\n" + "â"*70)
    print("  [v5.0] MODULE EXPERIMENT RESULTS  (OOS 2020â2026)")
    print("  Decision criteria: p<0.10 AND â¥4/7 year wins AND 2022 Sharpe â¥ V0")
    print("â"*70)
    ...
    print("\n  ââ V0âV5 Performance ââ")
    print(pd.DataFrame(v5_metrics).to_string(index=False))

    # Bootstrap Sharpe vs V0
    print("\n  ââ Bootstrap Sharpe Difference vs V0 (5000 resamples) ââ")
    ...
    for _vlabel in list(V5_VARIANTS.keys())[1:]:
        ...
        _sig    = "â sig" if _boot["significant"] else "â not sig"
        print(f"  V0 â {_vlabel:12s}: Î={_boot['diff']:+.3f}  p={_boot['p_value']:.3f}  "
              f"CI=[{_boot['ci_lo']:+.3f},{_boot['ci_hi']:+.3f}]  {_sig}")

    # Year-by-year for V0âV5
    _v5_yearly = yearly_breakdown(v5_oos)
    print("\n  ââ V0âV5 Year-by-Year Returns ââ")
    print(_v5_yearly.applymap(lambda v: f"{v:+.1%}" if not np.isnan(v) else "-").to_string())

    # Decision verdicts using pre-committed criteria
    print("\n  ââ Module Verdicts (PROMOTE / MARGINAL / SKIP) ââ")
    ...
    for _vlabel in list(V5_VARIANTS.keys())[1:]:
        ...
        print(f"  {_vlabel:12s}: sig={_p1}, yr_wins={_wins}/{_total}, "
              f"bear={_vx_bear_sh:.2f}â¥{_v0_bear_sh:.2f}={_p3}  â {_verdict}")
    ...
    _h90_sig  = "â sig" if _h90_boot["significant"] else "â not sig"
    print(f"\n  Bootstrap Sharpe (PS20 â PS20+H90):")
    print(f"    Î={_h90_boot['diff']:+.3f}  p={_h90_boot['p_value']:.3f}  "
    ...
    print(f"\n  H90 on Full Stack: sig={_h90_p1}, yr_wins={_h90_wins}/{_h90_total}, "
          f"bear={_ps20_h90_bear_sh:.2f}â¥{_ps20_bear_sh:.2f}={_h90_p3}  â {_h90_verdict}")
    ...
    plot_strategy_comparison(
        all_bt,
        f"BTC Combined Backtest v5.0  [{EVAL_START} â present]  (10 bps fees)",
